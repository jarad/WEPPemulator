...
#' get_all_wavg_exact(slp)

get_all_wavg_exact <- function(slp) {
...
#' slp <- read_slp(fpath_slp)
#' slp_extra <- tweak_slp(slp)
#' get_wavg_exact(slp_extra, weight = 0)
#' get_wavg_exact(slp_extra, weight = 1)
#' get_wavg_exact(slp_extra, weight = 2)
get_wavg_exact <- function(slp_extra, weight) {
    if (weight == 0) {
